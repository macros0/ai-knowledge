...
- **PDF** — текст и вложенные файлы через `pypdf`, рендеринг сканированных
  страниц через `pypdfium2`/PDFium.

PDF-публичный API не менялся: `parse_document()` и `Block` имеют тот же
контракт, что и для DOCX/XLSX. PDFium используется только для JPEG-рендеринга
страниц без текстового слоя; все вызовы PDFium сериализованы внутри процесса,
поскольку pipeline может разбирать несколько документов параллельно. OCR не
входит в пакет: скан остаётся изображением, которое может быть обработано
следующим уровнем сервиса. PyMuPDF не является зависимостью или fallback этого
пакета.
