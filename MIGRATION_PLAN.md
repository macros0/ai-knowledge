...
- **Дрейк тегов.** После миграции теги хранятся в: (1) `document_tags` (canonical), (2) `okf_concepts.tags` (per-concept), (3) Qdrant payload (для фильтра), (4) `.md` frontmatter. Canonical — `document_tags` + `okf_concepts.tags`; payload Qdrant и frontmatter `.md` — проекции, обновляются при правках тегов (Этап 4a roadmap).
