...
Задачи:
  ...
  2. С 2026-09-08 (немецкие ä/ö/ü/ß в TOKEN_RE) — пересборка sparse ПОСЛЕ СМЕНЫ
     ...
     токенайзера (sparse.TOKEN_EXTRA_LETTERS) или правки нормализации
     (sparse.normalize_for_tokens).

ВАЖНО — этот скрипт закрывает ТОЛЬКО ПОЛОВИНУ миграции. Тот же алфавит и ту же
нормализацию использует `deduplication._shingles`, поэтому смена формулы делает
minhash-подписи документов с затронутыми буквами несравнимыми со старыми: LSH-
бакеты продолжают ссылаться на подписи прежнего алфавита, и дедупликация по этим
документам молча перестаёт срабатывать. Полная процедура — ДВА прогона:

    python scripts/rebuild_sparse.py    # BM25-векторы в Qdrant
    python scripts/backfill_dedup.py    # minhash + LSH-бакеты в БД

Для чисто русско-английского корпуса второй прогон ничего не меняет (буквы вне
базового набора там не встречаются), но пропускать его нельзя — эффект зависит
от содержимого, а не от того, какой язык добавляли.
