...
class Locale(Base):
    ...
    `code` — BCP 47 / ISO 639 код (`ru`, `en`, ...). `status` управляет:
      - draft / active / disabled — активные участвуют в объединении стоп-слов
        для query-токенизации BM25 и принимаются `request_locale`;
      - активация требует хотя бы одного набора stopwords (kind=bm25); гейта
        «presence в UI-манифесте фронтенда» нет (двухуровневая модель с
        08.09.2026) — UI-языки задаются манифестом фронта отдельно, а при
        активации без runtime-словаря сидируется en-копия
        (`ui_dictionary.seed_english_copy`).
