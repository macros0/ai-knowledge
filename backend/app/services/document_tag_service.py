...
Qdrant-синхронизация — best-effort в ФОНОВОМ потоке (как dev_sync): каждый
set_payload в Qdrant на этой машине занимает ~2с, а на документ приходятся десятки
concept-точек с разными тегами — синхронное выполнение замораживало HTTP-ответ.
Недоступность Qdrant не роняет запрос, расхождение лечится следующим
...
#
# На этой машине каждый set_payload в Qdrant занимает ~2с, а на документ приходится
# десятки concept-точек с разными тегами — синхронное выполнение в HTTP-запросе
# замораживало ответ на десятки секунд (правка тегов «зависала»). Поэтому Qdrant
# обновляется в daemon-потоке: ответ возвращается мгновенно, поиск по тегам
# становится консистентным через несколько секунд. Синк идемпотентен (читает
# актуальное состояние БД) и сериализован по документу с dirty-флагом: если за
# время выполнения пришла новая правка — поток выполняет синк ещё раз.
