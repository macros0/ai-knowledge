"""Дедупликация при загрузке (Этап 4.2).

Три уровня:
  1. Точное совпадение (file_hash — SHA-256 байтов файла) — блокирующая проверка
     в upload-эндпоинте до создания записи.
  2. Почти идентичные (content_hash — SHA-256 нормализованного текста, либо
     LSH-strict с Jaccard >= порога) — side-by-side сравнение.
  3. Ревизии/похожие (LSH-loose, Jaccard в [loose, strict)) — неблокирующее
     уведомление после загрузки.
