...
## i18n (RU/EN/DE) — свой лёгкий механизм (05.09.2026)

Локализация клиентского UI (RU + EN; de — заглушка-ссылка на en с 08.09.2026),
переключатель `LocaleToggle` в топбаре сразу после `ThemeToggle` (циклический
RU → EN → DE). Переводы — **отдельные JSON/JS-словари** в
`frontend/src/i18n/locales/` (`ru.js` — фолбэк, `en.js` — ключи ⊆ ru, `index.js` —
манифест). **Новый язык = словарь + одна запись в `index.js`** (для de-стиля —
...
- **Алфавит токенайзера — европейская латиница (08.09.2026, 2 смены формулы).**
  ...
  RU-корпус не затрагивается). Язык с буквами вне набора = расширить константу +
  `python scripts/rebuild_sparse.py` (см. `docs/ADD_LANGUAGE.md` §4). В паре:
  `_slugify` — немецкие умлауты по DIN (ä→ae, ö→oe, ü→ue, ß→ss) + NFKD-снятие
  прочих диакритик (é→e); `deduplication._shingles` использует общий `TOKEN_RE`
  (regex, НЕ `tokenize()` — стоп-фильтр менял бы minhash всех RU-доков);
  `detect_language` выделяет «de» по ä/ö/ü/ß (немецкий без умлаутов остаётся «en» —
  лимит); `field_table._FIELD_NAME_RE` использует те же классы букв (обе регистры).
...
- **Немецкий UI-релиз — ЗАКРЫТО 08.09.2026 (безгейтная двухуровневая модель).**
  ...
  языки манифеста фронта (ru/en/de) — фроntend фильтрует активные по
  `SUPPORTED_LOCALES` (LocaleToggle.jsx), т.е. корпусный язык (fr/nl) активируется
  без релиза, но UI для него нет. de в манифесте — **ссылка на en-словарь**
  (`index.js`: `de: {messages: en}`, без файла de.js + `boot.js`): немецкий режим =
  английские строки + немецкие форматы дат/чисел до появления перевода (runtime
  override в админке). **Автосид en-копии при активации**: язык без runtime-override
  получает v1 = ПОЛНЫЙ en-словарь (`backend/app/i18n/ui_en.json`, генерируется
  `export-ui-keys.mjs` вместе с ui_keys.json; drift — i18n-тест). Функция
  `ui_dictionary.seed_english_copy` (вызывается из `activate_locale` после успешной
  активации, в try/except): skip ru/en, не перезаписывает существующий override,
  ...
  **Ожидаемое последствие (не баг):** пользователи с немецкой локалью браузера БЕЗ
  явного выбора языка (нет cookie okf.locale) переходят с ru-fallback на de-режим
  (английские строки) — явный выбор пользователя по-прежнему побеждает.
