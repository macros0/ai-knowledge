...
```powershell
.\scripts\start-all.ps1    # поднять весь стек: Qdrant, Ollama, backend, frontend
...
PID-файлы: `%TEMP%\opencode\{qdrant,ollama,backend,next}.pid`
Логи: `%TEMP%\opencode\{qdrant,ollama,python,node}-{out,err}.log`
