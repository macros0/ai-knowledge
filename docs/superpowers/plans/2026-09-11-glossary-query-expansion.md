...
2. В первом релизе автоматически расширять только термины, подтверждённые активной записью справочника, включая инфотипы. Структурный нормализатор может распознать неизвестный код, но не добавляет его в запрос. Это консервативное сужение v2: меньше охват, зато отключённые и неизвестные термины не обходят управление справочником.
...
Почему два флага: `0003` может быть полезной добавкой к уже распознанному `инфотип 3`, но отдельное `0003` не должно запускать распознавание. Короткие `ТР`, `PA`, `PY`, `ОМ` не допускаются ни как автоматические триггеры, ни как автоматически добавляемые общие формы.
...
Конечный список поддерживаемых префиксов: латинский `IT`, кириллический `ИТ`, английское `infotype`, русские `инфотип`, `инфотипа`, `инфотипе`, `инфотипу`, `инфотипом`. Между префиксом и числом — ноль или больше пробелов для IT/ИТ, один или больше для слов; допускается один `-` как разделитель. Число — только ASCII 0–9, от 1 до 4 цифр, нули добавляются слева.

| Вход | Кандидат | Автоматическое действие |
|---|---|---|
| `ИТ 0003`, `IT0003`, `it-3`, `инфотип 3`, `infotype 0003` | `IT0003` | Только если такой активный термин есть |
| `в инфотипе 3` | `IT0003` | Распознаётся span `инфотипе 3` |
| `3`, `0003` | Нет | Обычный запрос |
| `IT12345`, `IT-0003X`, `XIT0003`, `IT 12.5`, `IT 12,5` | Нет | Не брать допустимый префикс недопустимого числа |
| `IT 0000`, `IT 9999` | Форматный кандидат | Не утверждать существование: нужен активный термин |
...
Структурная нормализация резервирует эквивалентность: нельзя создать alias `инфотип 3` к иному термину. Конфликт проверяется на записи, не разрешается выбором «первый найденный» во время поиска.
...
| `GLOSSARY_SPARSE_EXPANSION_WEIGHT` | 0.35 — начальный параметр для проверки, не доказанный оптимум |
...
4. Sparse добавок строить тем же существующим хэшем через `to_sparse_vector`; значения новых индексов умножить на коэффициент 0.35.
...
Qdrant lexical retrieval всё ещё не обеспечивает точный поиск фразы. Добавка `0003` может найти посторонний числовой код: поэтому это opt-in alias с малым весом и отдельным негативным набором приёмки.
...
| `backend/scripts/probe_sources.py` | Паритет runtime и before/after для глоссария |
...
```python
@pytest.mark.parametrize("query", ["ИТ 0003", "it-3", "в инфотипе 3", "infotype 0003"])
def test_infotype_forms(query, glossary_term):
    plan = prepare_query(query, ui_locale="en", enabled=True)
    assert plan.original_query == query
    assert [t.canonical for t in plan.applied_terms] == ["IT0003"]

@pytest.mark.parametrize("query", ["ТР", "PA", "0003", "IT12345", "XIT0003", "IT 12.5"])
...
- [x] Тест: запрос `инфотип 3`, источник только с `IT0003` не удалён как unmatched.
...
- артефакты корпуса: `backend/scripts/probe-stage8-off.json` и
  `backend/scripts/probe-stage8-on.json` (локальные, не коммитятся).
- после исправления provenance выполнена матрица `252` вариантов (`42 ×
  ...
  `backend/scripts/probe-stage8-matrix-off.json` и
  `backend/scripts/probe-stage8-matrix-on.json`;
...
- отдельный отчёт `backend/scripts/probe-stage8-matrix-loss-report.json`
...
- причина исторических `8049 unresolved` в свежем на тот момент сравнении локализована: в
  `backend/scripts/glossary-probe-cases.json` отсутствуют mappings
...
- 12.09.2026 в `backend/scripts/glossary_probe_report.py` добавлены
...
- 12.09.2026 после согласованного ACL-fix выполнены cold SQL counter и
  ...
  `100/100`; артефакт — `backend/scripts/probe-stage8-parallel-retrieval-20260912.json`.
...
- 12.09.2026 выполнен отдельный trade-off бюджета алиасов `0/1/2/4` для
  ...
  `backend/scripts/probe-stage8-budget-tradeoff-20260912.json`.
...
- 12.09.2026 после исправления формата probe выполнен свежий однопроцессный
  ...
  `backend/scripts/probe-stage8-harnessfix3-summary-20260912.json`.
...
- 12.09.2026 подготовлен машинный черновик предметной разметки по 15
  ...
  артефакт `backend/scripts/probe-stage8-labeling-candidates-harnessfix4-20260912.json`.
- 12.09.2026 добавлен request-local cache токенов query для
  ...
  `backend/scripts/probe-stage8-harnessfix5-summary-20260912.json` и
...
- 12.09.2026 `/search` получил bounded hydration по фактическому размеру API-
  ...
  `backend/scripts/probe-stage8-search-bounded-off-20260912.json`,
  `backend/scripts/probe-stage8-search-bounded-on-20260912.json`,
  `backend/scripts/probe-stage8-search-bounded-quality-report-20260912.json`.
- 12.09.2026 исправлен probe-контракт: search-кейсы теперь передают в hydration
  ...
  `backend/scripts/probe-stage8-search-bounded-summary-20260912.json`,
  `backend/scripts/probe-stage8-search-bounded-sla-recheck-20260912.json`,
  `backend/scripts/probe-stage8-search-bounded-quality-report-20260912.json`.
...
|---|---|
| 10+ положительных | IT0003/ИТ 0003/инфотип 3, известная транзакция, проверенная фраза |
...
| **2. Нормализаторы и QueryPlan** | Правила инфотипов/кодов/aliases, снимок реестра, приоритеты, запреты, лимиты и feature flag | **2–3** | Этап 1 | Положительные и негативные формы распознаются детерминированно; no-match/disabled не меняют запрос | **✅ Завершён** |
...
| **7. Интерфейс, seed и инструкции** | Страница глоссария, редакторы, preview, AppliedTerms в чате/истории, локализация, начальное наполнение, эксплуатационная документация | **3–5** | Этапы 4, 5 и 6; QueryPlan этапа 2 для preview; согласованный предметный seed | Полный сценарий сопровождения доступен через UI; seed идемпотентен; роли и переводы проверены в браузере | **🟨 Реализация завершена; authenticated smoke частично пройден, полный сценарий ещё не закрыт** |
...
- подтверждение, какие числовые или короткие доменные формы допустимы как исходящие алиасы. По умолчанию они выключены.

Остальные решения уже предложены в этом документе: один глобальный словарь, ручное управление admin, editor только для переводов, автоматическое расширение только зарегистрированных активных терминов, первый релиз с UI, прямые переводы от оригинала, feature flag off до приёмки.
