...
**Статус:** этапы 0–8 выполнены. 15.09.2026 владелец принял сохранённые
примеры и измерительную базу как достаточную проверку текущего выпуска и
исключил строгий относительный SLA и дополнительную разметку из release-gate.
Расширение включено на LXC 102 через штатный runtime env с перезапуском backend
и проверкой `/health`. Детали прежних измерений сохранены в
[плане приёмки этапа 8](2026-09-11-glossary-stage8-acceptance.md).
...
- [x] Проверить в браузере сценарий admin/editor/user, узкую ширину, клавиатуру, плашку нулевого результата и старую историю. Владелец подтвердил выполнение этой проверки при создании измерительной базы 15.09.2026.
...
- [x] Зафиксировать пользовательские примеры и негативные критерии: сохранённые примеры и измерительная база приняты владельцем 15.09.2026 как достаточный reference текущего выпуска; дополнительная разметка не требуется.
...
- [x] Зафиксировать результаты времени и нагрузки как наблюдаемые метрики, не как release-SLA; отключение термина и глобального флага с рестартом ранее проверено в изолированном контуре.
- [x] Включить флаг и провести smoke после рестарта/деплоя: 15.09.2026 на LXC 102 установлен `GLOSSARY_QUERY_EXPANSION_ENABLED=true`, `okf-knowledge.service` перезапущен, backend health — `ok`.

**Приёмка:** завершена решением владельца 15.09.2026. Предварительные
off/on-прогоны, негативные случаи и измерительная база сохранены как reference.
Строгий относительный performance SLA и дополнительная предметная разметка
сняты с release-gate; они могут быть возвращены только отдельным новым решением.
...
5. Deploy backend с безопасным значением флага false для новой инсталляции; проверить старый клиент и старые сообщения.
...
8. Выполнить корпусную проверку и включить `GLOSSARY_QUERY_EXPANSION_ENABLED` штатной конфигурацией с перезапуском/деплоем. Выполнено 15.09.2026 на LXC 102; backend перезапущен и `/health` вернул `ok`.
9. Smoke: запрос с alias, no-match, два термина, запрещённая аббревиатура, неизвестный язык, история, фильтр source locale, отключение записи. Выполнен владельцем при создании измерительной базы.
...
| **2. Нормализаторы и QueryPlan** | Правила инфотипов/кодов/aliases, снимок реестра, приоритеты, запреты, лимиты и feature flag | **2–3** | Этап 1 | Положительные и негативные формы распознаются детерминированно; no-match/disabled не меняют запрос | **✅ Завершён: редактируемые правила диапазонов и префиксов реализованы** |
...
| **7. Интерфейс, seed и инструкции** | Страница глоссария, редакторы, preview, AppliedTerms в чате/истории, локализация, начальное наполнение, эксплуатационная документация | **3–5** | Этапы 4, 5 и 6; QueryPlan этапа 2 для preview; согласованный предметный seed | Полный сценарий сопровождения доступен через UI; seed идемпотентен; роли и переводы проверены в браузере | **✅ Завершён** |
| **8. Корпусная приёмка и выпуск** | Общий probe, сравнение off/on, качество и производительность, полная регрессия, проверка миграции/отката, включение и smoke | **2–3** | Этапы 0–7; пользовательские reference-примеры и измерительная база | Владелец принял проверку; функция включена штатной конфигурацией | **✅ Завершён 15.09.2026: LXC 102, backend health ok** |
