...
Дата: 11.09.2026. Статус: техническая часть probe и UX-индикации обновлена;
миграционный прогон и базовые frontend/backend проверки закрыты. Предметная
приёмка, последовательный performance SLA и выпуск не завершены.

## Общие ограничения

- До прохождения технической и предметной приёмки release-конфигурация
  `GLOSSARY_QUERY_EXPANSION_ENABLED=false`. Локальный `.env` может временно
  иметь `true` только для визуального smoke; это не считается включением в
  выпуск.
...
- Не менять критерии после просмотра after, чтобы получить проходящий результат. Коррекция ошибочной разметки требует причины, новой версии и повторного сравнения обеих сторон.
...
- Отсутствие данных или обязательного прогона означает «не проверено» и блокирует соответствующий пункт, а не даёт PASS.

## Исходные данные и границы доказательств

Есть 42 сценария и локальные `backend/scripts/probe-stage8-off.json` / `probe-stage8-on.json`. Все сценарии по умолчанию используют hybrid. Набор содержит ожидания canonical, но не утверждённые релевантные источники. Seed содержит 7 терминов; основной план требует предметно проверенные 10–15.
...
**Файлы:** изменить `backend/scripts/probe_sources.py`, `backend/tests/test_probe_sources.py`; при необходимости явной передачи provenance — `backend/app/services/context_builder.py` и его тесты.
...
- [x] Сохранять отчёт сравнения в отдельный JSON; существующие baseline не перезаписывать. Любое неожиданное canonical считать ошибкой, включая canonical вне списка forbidden. Проверено на `backend/scripts/probe-stage8-ux-loss-report.json`.
...
**Файлы:** создать `backend/scripts/glossary_probe_report.py`, `backend/tests/test_glossary_probe_report.py`, `docs/superpowers/reports/2026-09-11-glossary-stage8-loss-review.md`.
...
**Файлы:** изменить `backend/scripts/glossary-probe-cases.json`, соответствующий JSON в `backend/seeds/`, `backend/tests/test_probe_sources.py`; создать `docs/superpowers/reports/2026-09-11-glossary-stage8-labels.md`.
...
**Файлы:** `backend/scripts/probe_sources.py`, JSON-артефакты с уникальным run_id, отчёт из 8.2. Исправления приложения — только в локализованном участке с регрессионным тестом.
...
- [x] 12.09.2026: после согласованного ACL-fix выполнены cold SQL counter (`cold off=0`, `cold on=5`, `warm on=0`) и retrieval-only parallel smoke (`5×20`, off/on по 100 запросов, ошибок и acceptance failures `0`, embedding calls `100/100`); артефакт `backend/scripts/probe-stage8-parallel-retrieval-20260912.json`.
...
- [x] 12.09.2026: выполнен trade-off лимита добавляемых алиасов `0/1/2/4`; лимиты `0/1/2` меняют retrieval source identities (до `532` raw-потерь относительно default `4`) и отклонены без утверждённых relevance labels. Default не менялся; артефакт — `backend/scripts/probe-stage8-budget-tradeoff-20260912.json`.
...
- [x] 12.09.2026: категориальный разбор harnessfix4 подтвердил, что BM25 overhead сосредоточен в positive/compound/multilingual/filter-isolation кейсах с расширенным raw-пулом; negative/legacy без расширения почти не деградируют. Артефакт: `backend/scripts/probe-stage8-harnessfix4-bm25-breakdown-20260912.json`. Изменение alias-budget, sparse-весов или лимита кандидатов отложено до labels.
- [x] 12.09.2026: подготовлен машинный черновик предметной разметки по 15 positive/compound кейсам: 209 уникальных top-5 кандидатов с точной identity, наблюдениями API/mode и excerpt из canonical БД; labels не назначались. Артефакт: `backend/scripts/probe-stage8-labeling-candidates-harnessfix4-20260912.json`.
...
- [x] 12.09.2026: добавлен read-only валидатор `backend/scripts/validate_stage8_labels.py` и 4 regression-теста; текущий файл cases корректно даёт `BLOCKED` (`15` quality-кейсов, `0` complete), labels не назначались.
- [x] 12.09.2026: visibility lookup и canonical hydration объединены в одну DB-сессию для `/search`, `/chat` и probe; полный 2-warmup/5-pair протокол на 210 наблюдениях на сторону сохранил `acceptance_failures=[]` и дал `search/bm25 +72.31%/+16.77%`, `chat/bm25 +103.88%/+32.16%` (avg/p95), но SLA остаётся FAIL. Сравнение со старым protocol-1: `0` изменений финальных identity/kind/title для off/on; артефакт `backend/scripts/probe-stage8-combined-hydration-summary-20260912.json`.
...
- [x] 12.09.2026: bounded canonical hydration ограничила SQL-выборку границами ответа (`4000/6000` символов); новый 2-warmup/5-pair протокол сохранил `acceptance_failures=[]` и `0` semantic identity differences, но BM25 SLA остался FAIL (`search +76.17%/+10.39%`, `chat +108.55%/+26.16%`, avg/p95). Затронутые тесты — `142 passed, 4 warnings`; артефакт `backend/scripts/probe-stage8-bounded-hydration-summary-20260912.json`.
- [x] 12.09.2026: выполнен SLA recheck по единому критерию плана (+10% для average и p95): `3/6` пар PASS; `search/bm25` FAIL average/p95, `chat/bm25` FAIL average/p95, `chat/hybrid` FAIL p95 (`+11.78%`). Артефакт `backend/scripts/probe-stage8-bounded-hydration-sla-recheck-20260912.json`; критерий и release gate не изменялись.
- [x] 12.09.2026: quality report пересчитан на bounded protocol-1 текущего кода: `acceptance_failures=[]`, `90 BLOCKED` и `162 NOT_APPLICABLE` из-за отсутствия утверждённых labels; артефакт `backend/scripts/probe-stage8-bounded-hydration-quality-report-20260912.json`.
- [x] 12.09.2026: категориальный BM25-разбор bounded-протокола подтвердил, что рост raw-пула сосредоточен в positive/compound/multilingual/filter-isolation, а negative/legacy не меняются; в chat compound/multilingual дополнительно растёт context-filter. Артефакт `backend/scripts/probe-stage8-bounded-hydration-bm25-breakdown-20260912.json`; изменение пула отложено до labels.
...
- [x] 12.09.2026: подготовлен дополнительный `draft_unapproved` packet для final-only изменений после search hydration: `75` case-run (`21` off, `54` on), полных raw source losses — `0`; labels автоматически не назначались. Артефакт: `backend/scripts/probe-stage8-search-bounded-final-only-review-20260912.json`.
