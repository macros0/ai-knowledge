...
| `services/pipeline.py` | `_threads`, `_abort_events` | Прерывание (удаление, purge) доходит только до той реплики, в которой идёт обработка. В чужой реплике словарь пуст — пайплайн допишет статус и векторы уже удалённому документу |
