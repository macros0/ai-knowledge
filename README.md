...
4. **RAG-хранилище** — **Qdrant**: dense-вектор (безымянный, семантический) + sparse-вектор `"sparse"` (лексический, BM25) + payload (метаданные из YAML). Dense и sparse строятся из `title + "\n" + content[:okf_max_concept_chars]` — title содержит коды/номера разделов (например, «12410»), которые иначе не попадали в индекс и концепт не находился по поиску по коду. Payload хранит `content[:cap]` (то, что видит LLM в контексте и пользователь в сниппете). Три режима поиска: плотный (`dense`), ключевые слова (`bm25`), гибридный (`hybrid`, префикс dense+sparse + fusion RRF).
...
| `SEARCH_MODE_DEFAULT` | `hybrid` | Режим поиска по умолчанию: `dense` (семантический), `bm25` (ключевые слова), `hybrid` (dense+sparse RRF) |
...
- **`dense`** — семантический поиск по dense-вектору (как было ранее). Находит концепты по смыслу, игнорируя точное совпадение слов.
- **`bm25`** — лексический поиск по sparse-вектору (ключевые слова IDF/BM25). Находит точные совпадения терминов; счёт нормализован в ответе (максимум = 1.0).
- **`hybrid`** (рекомендуемый) — dense-префикс + sparse-префикс с fusion RRF. Лучший компромисс между семантикой и точностью ключевых слов.

В панели чата режим выбирается переключателем над полем ввода концептов.

### Миграция существующей коллекции

Если у вас уже была коллекция Qdrant с безымянным dense-вектором (версии до введения search modes), при старте сервиса автоматически:
1. Добавляется именованный sparse-вектор `"sparse"` в существующую коллекцию через `create_vector_name` (без даунтайма, dense не затрагивается).
2. Запускается фоновый бэкфилл: sparse-векторы вычисляются из OKF-бандлов на диске (`data/okf_bundles/*/`) и догружаются через `update_vectors` (сохраняет dense и payload существующих точек).

Бэкфилл идемпотентен и не требует ручного реиндекса или перезагрузки документов.
