...
4. **RAG-хранилище** — **Qdrant**: dense-вектор (безымянный, семантический) + sparse-вектор `"sparse"` (лексический, BM25) + payload (метаданные из YAML). Три режима поиска: плотный (`dense`), ключевые слова (`bm25`), гибридный (`hybrid`, префикс dense+sparse + fusion RRF).
5. **Умный поиск** — эмбеддинг запроса → поиск в Qdrant в выбранном режиме → LLM формирует ответ по найденным концептам с указанием источников. Выбор режима — селектор в панели чата или параметр `mode` в API.
...
| `CHAT_TOP_K_MAX` | `10` | Макс. число концептов в ответе (верхняя граница степпера «Другое») |
| `CHAT_TOP_K_DEFAULT` | `5` | Число концептов по умолчанию (кнопка «Стандартно») |
| `CHAT_TOP_K_PRESETS` | `4,5,10` | Кнопки быстрого выбора числа концептов (разделяются запятыми) |
