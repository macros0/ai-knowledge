...
После resume такой чанк обрабатывается заново: обычно `json_repair` восстанавливает
JSON автоматически, а если нет — файл дампа в `data/debug/` позволяет увидеть
точную причину (обрезание, невалидные символы, артефакт провайдера).
