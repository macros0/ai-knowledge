...
### Парсинг JSON из ответа LLM (каскад + диагностика)
...
2. **Восстановление обрезанного хвоста** — ищет валидный префикс + закрывающую
   скобку в последних 1000 символах (снимает обрезание по `max_tokens`).
3. **`json_repair`** (библиотека, пакет `json-repair`) — исправляет
   неэкранированные управляющие символы, лишние/пропущенные запятые,
   обрезанные массивы, мусор и markdown-обёртку вокруг JSON. Это стандартное
   индустриальное решение для LLM-вывода.
4. **Дамп + понятная ошибка** — если JSON не восстановлен ни одним из способов,
   ...
   (каталог в `.gitignore`), а документ переходит в `paused` с ошибкой,
   содержащей первые 200 символов ответа и путь к дампу.

После resume такой чанк обрабатывается заново: обычно `json_repair` восстанавливает JSON
автоматически, а если нет — файл дампа в `data/debug/` позволяет увидеть точную причину
(обрезание, невалидные символы, артефакт провайдера).

**Прозрачность в логах:** в журнале (`backend/logs/` или stdout) сразу видно, какой
уровень сработал — `json.loads` успешен (тихо), `_recover_truncated` (INFO: «хвост
обрезан»), `json_repair` (INFO: «успешно восстановлен»), дамп (WARNING: путь к файлу).
Это позволяет отличать стабильно чистые ответы модели от «спасённых» и диагностировать
проблемные провайдеры/модели.
